...
for i in sdms/*/nov/ ; do #
    ...
    if test "$(ls -A "$target")"; then #
        ...
        if test $(ls -A $i/*.csv); then #
            ...
            Rscript ebutterfly-sdm/scripts/run-sdm.R $i*.csv $name-nov-output $i/output #
            #Rscript ebutterfly-sdm/scripts/run-sdm-algo.R $i*.csv $name-nov-output $i/output #
        fi #
    fi #
done #
