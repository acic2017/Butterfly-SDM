...
for i in sdms/*/jan/ ; do #
    ...
    if test "$(ls -A "$target")"; then #
        ...
        if test $(ls -A $i/*.csv); then #
            ...
            Rscript ebutterfly-sdm/scripts/run-sdm.R $i*.csv $name-jan-output $i/output #
            #Rscript ebutterfly-sdm/scripts/run-sdm-algo.R $i*.csv $name-jan-output $i/output #
        fi #
    fi #
done #
