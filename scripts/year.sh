...
#
cd /extra/jhanson3/midterm/main #TODO: change the directory
for i in sdms/*/all/ ; do #
    target=$i/output #
    if test "$(ls -A "$target")"; then #
        ...
        if test $(ls -A $i/*.csv); then #
            ...
            Rscript ebutterfly-sdm/scripts/run-sdm.R $i*.csv $name-all-output $i/output #
            #Rscript ebutterfly-sdm/scripts/run-sdm-algo.R $i*.csv $name-all-output $i/output #
        fi #
    fi #
done #